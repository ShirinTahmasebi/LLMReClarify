...
def set_template(args):
    if args.dataset_code == None:
        ...
        dataset_code = {'1': 'ml-100k', '2': 'ml-1m', 'b': 'beauty', 'g': 'games'}
        args.dataset_code = dataset_code['2'] #input('Input 1 for ml-100k, 2 for ml-1m b for beauty and g for games: ')]

    is_movielens = args.dataset_code == 'ml-100k' or args.dataset_code == 'ml-1m'
    ...
    if 'llm' in args.model_code: 
        batch = 16 if is_movielens else 12
        args.lora_micro_batch_size = batch
    else: 
        batch = 16 if is_movielens else 64
...
parser.add_argument('--llm_max_history', type=int, default=18)
...
parser.add_argument('--llm_retrieved_path', type=str, default=f'{EXPERIMENT_ROOT}/lru/ml-1m/')
parser.add_argument('--llm_cache_dir', type=str, default=None)
parser.add_argument('--llm_gt_pos', type=int, default=-1)
parser.add_argument('--llm_trained_model_path', type=str, default=f'{EXPERIMENT_ROOT}/Llama-2-7b-hf/ml-1m/checkpoint-2200')
################
# Lora
################
