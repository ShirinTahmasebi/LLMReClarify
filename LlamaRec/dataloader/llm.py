...
def fetch_candidates(gt_pos, scores_all_users, gt_all_users, candidate_size=20):
    if gt_pos <= 0:
        gt_pos = random.randint(1, candidate_size)
    gt_index = gt_pos - 1
    ...
    for u, (p,l) in enumerate(zip(scores_all_users, gt_all_users), start=1):
        ...
        candidates_all_users.append(top_recs.tolist())
...
class LLMDataloader():
    def __init__(self, args, dataset):
        ...
        self.gt_pos = args.llm_gt_pos
        
        args.num_items = self.item_count
        self.max_len = args.llm_max_history
        
        self.tokenizer = AutoTokenizer.from_pretrained(
            args.llm_base_tokenizer, cache_dir=args.llm_cache_dir, use_auth_token=os.environ['HUGGINGFACE_TOKEN'])
        ...
        # Put the true label in the self.gt_pos-th position
        val_candidates_injected = fetch_candidates(self.gt_pos, self.val_probs, self.val_labels)

        self.val_users_all = [u for u, (_, _) in enumerate(zip(self.val_probs, self.val_labels), start=1)]
        ...
        # self.val_candidates = val_candidates_injected
        ...
        # Put the true label in the self.gt_pos-th position
        test_candidates_injected = fetch_candidates(self.gt_pos, self.test_probs, self.test_labels)
        self.test_users_all = [u for u, (_, _) in enumerate(zip(self.test_probs, self.test_labels), start=1)]
        self.test_users = [u for u, (p, l) in enumerate(zip(self.test_probs, self.test_labels), start=1) \
                          if l in torch.topk(torch.tensor(p), self.args.llm_negative_sample_size+1).indices]
        # self.test_candidates = test_candidates_injected
        ...
        # self.non_test_users = [u for u, (_, _) in enumerate(zip(self.test_probs, self.test_labels), start=1) \
                            #    if u not in self.test_users]
        ...
        print(f"The number of all users in test set: {len(self.test_users_all)}")
        print(f"The number of valid users in test set: {len(self.test_users)}")
        
        print(f"The number of all users in val set: {len(self.val_users_all)}")
        print(f"The number of valid users in val set: {len(self.val_users)}")
